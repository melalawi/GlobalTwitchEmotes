...
//Only the functions defined in exports can be 'exported' to the other background scripts
//'exports' does not exist in chrome, however, and as a result attempting to call it will result in a crash
...
function Background(browser_interface) {
    ...
    this.initialize = function() {
        ...
        emote_manager = new EmoteManager(send_data);
        local_settings.load_data();
    };
    
    //separated parameters because we are unaware of tab format until runtime
    this.add_tab = function(tab, url) {
        if (tab && url) {
            //passes test
            if (url_check(url)) {
                tab_queue.push(tab);
    ...
    function send_data() {
        tab_queue.forEach(function(tab){
            interface.script_injector(tab, function(){
                //upon successful injection
                tab_queue.splice(tab, 1);},
                //message to send
                {message: 'data', list: emote_manager.emote_list, dynamic: local_settings.replace_emotes_dynamically});
        });
    };
...
function EmoteManager(success) {
    ...
    this.add_emote = function(emote_name, emote_url) {
        if (emote_name !== undefined && emote_url !== undefined) {
            if (self.is_valid_emote(emote_name)) {
                ...
                emote_html = '<img src="' + emote_url + '" title="' + emote_name + '" alt="' + emote_name + '" style="display: inline;">';

                self.emote_list[emote_name] = emote_html;
            }
        }
    };
...
function Settings(access, success) {
    ...
    this.build_data = function(data) {
        ...
        self.emote_filter_list = self.emote_filter_list.split('\n');
        self.site_filter_list = self.site_filter_list.split('\n');
    };
}

function parse_global_json(json, func) {
    func('Kappa', 'http://static-cdn.jtvnw.net/jtv_user_pictures/chansub-global-emoticon-ddc6e3a8732cb50f-25x28.png');

    for (var parse_emote in json) {
        func(parse_emote, json[parse_emote]['url']);
    }
}
    ...
    for (var parse_channel in json) {
        for (var parse_emote in json[parse_channel]['emotes']) {
            if (filtered_channels.indexOf(parse_channel.toLowerCase()) === -1) {
                func(parse_emote, json[parse_channel]['emotes'][parse_emote]);
            }
        }
    }
}
...
function parse_betterttv_json(json, func) {
    for (var parse_emote in json) {
        func(json[parse_emote]['regex'], json[parse_emote]['url']);
    }
}
...
function include_smilies(mode, monkeys, func) {
    ...
    for (var index in smilies) {
        func(index, smilies[index]);
    }

    //monkey support
    if (monkeys) {
        for (var index in monkey_smilies) {
            func(index, monkey_smilies[index]);
        }
    }
