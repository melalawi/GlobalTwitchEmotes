...
if (typeof exports === 'object') {
    exports.newBackground = function(adaptor) {
        return new GTE_BACKGROUND.Manager(adaptor);
    };
}
...
var CONTENTSCRIPTS = {
    js: ['./data/js/jquery-2.1.4.min.js', './data/js/jquery.tipsy.js', './data/js/script.js'],
    css: ['./data/css/gte.css']
};
...
function Manager(browserAdaptor) {
    var backendManager,
        ...
        emotesReady = new browserAdaptor.Deferred.Deferred();

    emotesReady.then(pushEmotes);

    this.initialize = function() {
        CONTENTSCRIPTS.js.push(adaptor.scriptFile);
    ...
    function messageHandler(tab, message, data, responseCallback) {
        switch (message) {
            ...
            case MESSAGES.SAVE_SETTINGS:
                if (backendManager) {
                    var saveConfirmed = new browserAdaptor.Deferred.Deferred();
    ...
    function serveTab(tab) {
        if (emotesReady.state() === 'resolved') {
            ...
            if (tabURL.match(/(^http)/gi) && isValidURL(tabURL)) {
                adaptor.initializeTab(tab, CONTENTSCRIPTS, backendManager.getTabData());
            }
        }
    }
...
function BackendManager(browserAdaptor) {
    ...
    function buildEmoticons(userSettings, onComplete) {
        ...
        //on first time only
        adaptor.Deferred.when(deferredArray, function(){
        ...
        if (currentHost) {
            ...
            if (channelList) {
                ...
                for (var listIndex = 0; listIndex < channelList.length; ++listIndex) {
                    nextDeferred = new adaptor.Deferred.Deferred();
...
function EmoteSet(emoteHostObject, deferred, browserAdaptor, filterCallback) {
    ...
    this.initialize = function(channelName) {
        retrieveJSON(channelName);
    };

    function retrieveJSON(channelName) {
        var setURL;

        setURL = hostObject.getBaseURL(channelName);
        ...
        adaptor.XMLRequest(setURL, parseResponse, function(){
            ...
            setTimeout(function() { retrieveJSON(channelName); }, 5000);
        });
    }

    function parseResponse(responseText) {
        hostObject.parse(JSON.parse(responseText), filter);
